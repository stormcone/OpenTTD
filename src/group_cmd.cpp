...
std::tuple<CommandCost, GroupID> CmdCreateGroupAutogenName(DoCommandFlag flags, VehicleID vehicle_id, GroupID parent_group, bool add_shared)
{
	...
	if (unique_orders.empty()) return { CommandCost(STR_ERROR_GROUP_CAN_T_CREATE_NAME), INVALID_GROUP };
	...
	if (_settings_client.gui.autogen_group_name == 1) { // Use station names
		...
		if (station_last == nullptr || station_first == nullptr) return { CommandCost(STR_ERROR_GROUP_CAN_T_CREATE_NAME), INVALID_GROUP };
	...
	GroupID new_g = std::get<1>(ret);
	CmdAlterGroup(flags, AlterGroupMode::Rename, new_g, 0, str);
